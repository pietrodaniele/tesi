...
\tableofcontents
		...
		\section{ATLAS}	
			...
			The interactions in the ATLAS detectors create an enormous flow of data. To digest the data, ATLAS uses an advanced “trigger” system to tell the detector which events to record and which to ignore. Complex data-acquisition and computing systems are then used to analyse the collision events recorded. At 46 m long, 25 m high and 25 m wide, the 7000-tonne ATLAS detector is the largest volume particle detector ever constructed. 
				...
				\subsection{Trigger system}
					...
					\begin{figure}
						\centering
						\subfloat[]{\includegraphics[width=.42\textwidth]{tesi_images/L1.png}} \quad
						\subfloat[]{\includegraphics[width=.45\textwidth]{tesi_images/HLT.png}} 
	...
	\chapter{BDT: Electron and photon classification}	
		...
		After removing pile-up, the dataframe is splitted in the train (80\%) and the test (20\%) sets, using a Scikit Learn function. The train dataset is used to train the algorithm, while the test dataset is used as model input. If we did not do this split, we would introduce a bias.
		...
		The purpose of creating these models is to explore the possibility of
		replacing the current classification algorithm with a GBDT for all electrons and photons candidates. So the models performance are compared with each other and with the old and new ambiguity tool. The problem of a GBDT in reconstrution is the memory cost: all particles must have both reconstruction. So the gain in performance must justify the memory cost.
		...
		\section{BDT training}
			...
			\subsection{Discriminating features}
				...
				\subsubsection{Electron features}
					...
					\begin{itemize}
						\item \texttt{el\_track\_ep},
						\item \texttt{el\_trackz0},
						\item \texttt{el\_cl\_pt}, 
						\item \texttt{el\_cl\_eta}
					\end{itemize}
				...
				\subsubsection{Photon features}
					...
					\begin{itemize}
						\item \texttt{ph\_zconv} and \texttt{ph\_Rconv}: these characteristics contain information of the conversion point in the coordinates R and z. \texttt{True\_el/reco\_ph} have a conversion point close to the point of interaction as their track starts from the beginning.
						\item \texttt{ph\_pt1conv}, \texttt{ph\_pt2conv} and \texttt{ph\_ptconv}. All these three features have been scaled with \texttt{ph\_cl\_pt} to eliminate all kinematics information 
						\item \texttt{pt1conv/ptconv} contains information on conversion symmetry. This characteristic for \texttt{true\_el/reco\_ph} tends to 1, since most of them have a single track.
					\end{itemize}
			...
			\subsection{Parameters and Hyperparameter optimization}
				Parameters and hyperparameter regulate training and determine the performance of the model created. Hyperparameters are LGMB adjustable parameters chosen for the training of a model, which regulate the training process. 
			...
			\subsection{Training and Model}
				Once the hyperparameter optimisation is complete, algorithm training can begin. It is based on a LGBM function \texttt{lgb.train}, which has as input: parameters set, lgb train dataset, the maximum number of training steps, validation sets,  early stopping rounds. The validation sets are the sets on which the loss function is calculated during training. For training to create the model they are \texttt{lgb\_train} and \texttt{lgb\_test}, while hyperparameters optimization they are \texttt{lgb\_train} and \texttt{lgb\_valid}, as said before. To prevent overfitting, the model will train until the validation score stops improving. Validation score needs to improve at least every \texttt{early\_stopping\_rounds} round(s) to continue training. [riferimento alla documentazione LGBM]
